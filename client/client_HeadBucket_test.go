...
func TestClient_HeadBucket(t *testing.T) {
	t.Run("required", func(t *testing.T) {
		t.Run("bucket", func(t *testing.T) {
			...
			require.EqualError(t, err, "HeadBucket: bucket is mandatory")
		})
	})
	...
	t.Run("request", func(t *testing.T) {
		t.Run("500", func(t *testing.T) {
			...
			headBucketOutput, err := c.HeadBucket(
				context.Background(),
				&HeadBucketInput{
					Bucket: "my-pouet-lv123",
				},
			)
			require.EqualError(t, err, `HeadBucket: unexpected http line "500 Internal Server Error"`)
			require.Nil(t, headBucketOutput)
		})
		...
		t.Run("VHostStyle", func(t *testing.T) {
			...
			headBucketOutput, err := c.HeadBucket(
				...
				&HeadBucketInput{
					Bucket:              "my-pouet-lv123",
					ExpectedBucketOwner: ToPointer("theOwner"),
				},
			)
			require.NoError(t, err)

			expectedRequest := "HEAD / HTTP/1.1\r\n" +
				"Host: " + ts.Listener.Addr().String() + "\r\n" +
				"User-Agent: Go-http-client/1.1\r\n" +
				"X-Amz-Expected-Bucket-Owner: theOwner\r\n" +
				"\r\n"
			require.Equal(t, []byte(expectedRequest), requestDump)

			expectedOutput := &HeadBucketOutput{
				BucketRegion:     ToPointer("BucketRegion"),
				AccessPointAlias: ToPointer("AccessPointAlias"),
			}
			require.Equal(t, expectedOutput, headBucketOutput)
		})
		...
		t.Run("PathStyle", func(t *testing.T) {
			...
			expectedRequest := "HEAD /my-pouet-lv123 HTTP/1.1\r\n" +
				"Host: " + ts.Listener.Addr().String() + "\r\n" +
				"User-Agent: Go-http-client/1.1\r\n" +
				"X-Amz-Expected-Bucket-Owner: theOwner\r\n" +
				"\r\n"
